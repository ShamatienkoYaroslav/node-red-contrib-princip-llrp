...
var llrpMain = function (config) {
  ...
  var bCloseConnection = new Buffer('040e0000000a00000000', 'hex');
	...
	this.connect = function () {
		...
		socket.setTimeout(60000, function () {
			if (self.connected){
				...
				process.nextTick(function () {
					...
					eventEmitter.emit('timeout', new Error('Connection timeout'));
				});
			};
		});
		...
		client = socket.connect(port, ipaddress, function () {
			...
			process.nextTick(function () {
				eventEmitter.emit('connect','connected: '+ipaddress + ':' + port);
			});
		});
		...
		client.on('end', function () {
			...
			if (log) {
				c('end client disconnected');
			}
			process.nextTick(function () {
				self.connected = false;
				eventEmitter.emit('disconnect', new Error('Client disconnected.'));
			});
		});
		...
		client.on('error', function (err) {
			...
			process.nextTick(function () {
				self.connected = false;
				eventEmitter.emit('error', "connection error: "+err);
			});
		});
	};
  ...
  this.disconnect = function() {
		self.connected = false;
		eventEmitter.removeAllListeners('princip-answer');
		eventEmitter.removeAllListeners('princip-message');
    process.nextTick(function() {
				...
				parametersKeyValue.forEach(function (decodedParameters) {
					...
					if (decodedParameters.type === parameterC.TagReportData) {
						...
						if (typeof subParameters[parameterC.LastSeenTimestampUTC] !== 'undefined') {
							tag.lastSeenTimestampUTC = subParameters[parameterC.LastSeenTimestampUTC];
						}
	...
	eventEmitter.on('princip-message', function(message) {
	...
	function emitMessage(message) {
		eventEmitter.emit('princip-answer', message);
	}
